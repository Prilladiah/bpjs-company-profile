...
import type { Galeri, GaleriKategori } from "@/types";

const EMPTY_FORM = { keterangan: "", kategori: "KINERJA_UTAMA" as GaleriKategori, foto: "", urutan: 0 };
...
export default function KelolaGaleriPage() {
  ...
  const load = useCallback(async (query = "") => {
    setLoading(true);
    const res = await fetch(`/api/galeri${query ? `?q=${encodeURIComponent(query)}` : ""}`);
  ...
  function openEdit(item: Galeri) {
    setEditing(item);
    setForm({ keterangan: item.keterangan, kategori: item.kategori, foto: item.foto, urutan: item.urutan });
    setModalOpen(true);
  }
  ...
  return (
    <div>
      <h1 className="mb-6 text-2xl font-bold text-gray-900">Kelola Galeri</h1>
        ...
        <table className="w-full text-left text-sm">
          ...
          <tbody className="divide-y divide-gray-100 bg-white">
            ...
            {!loading && list.length === 0 && (
              <tr><td colSpan={4} className="px-4 py-6 text-center text-gray-400">Belum ada foto galeri</td></tr>
            )}
            {list.map((item) => (
              <tr key={item.id}>
                <td className="px-4 py-3">{item.keterangan}</td>
                <td className="px-4 py-3">{GALERI_KATEGORI_LABEL[item.kategori]}</td>
      ...
      {modalOpen && (
        <Modal title={editing ? "Edit Galeri" : "Tambah Galeri"} onClose={() => setModalOpen(false)}>
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <select
                required
                value={form.kategori}
                onChange={(e) => setForm({ ...form, kategori: e.target.value as GaleriKategori })}
                className="w-full rounded-md border border-gray-300 px-3 py-2 text-sm outline-none focus:border-brand-blue"
              >
                {Object.entries(GALERI_KATEGORI_LABEL).map(([value, label]) => (
